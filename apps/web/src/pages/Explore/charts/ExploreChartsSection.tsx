...
function VolumeChartSection({ chainId }: { chainId: number }) {
  ...
  if (isSmallScreen) {
    return (
      <MinimalStatDisplay
        title={<Trans>Niza DEX volume</Trans>}
...
function TVLChartSection({ chainId }: { chainId: number }) {
  ...
  if (isSmallScreen) {
    ...
    return <MinimalStatDisplay title={<Trans>Niza DEX TVL</Trans>} value={currentTVL} />
  }

  return (
    <SectionContainer>
      <SectionTitle>
        <Trans>Niza DEX TVL</Trans>
      </SectionTitle>
      {(() => {
        if (dataQuality === DataQuality.INVALID) {
          const errorText = loading ? undefined : (
